...
class SportmonksFixtureSyncService:
    def __init__(self, *, client, publisher, predictions_enabled=False):
        self.client = client; self.publisher = publisher; self.predictions_enabled = bool(predictions_enabled)

    async def _enrich_market_data(self, fixture):
        fixture_id = str(fixture.get("id") or "")
        if not fixture_id: return fixture
        enriched = dict(fixture); enriched.setdefault("meta", {})
        features = {}
        try:
        ...
        if self.predictions_enabled:
            try:
                ...
                for key, value in predicted.items(): features.setdefault(key, value)
    ...
    async def sync_between(self, *, start_date, end_date, include="participants;scores;state;league", max_pages=100):
        ...
        async for raw_fixture in self.client.iter_fixtures_between(start_date, end_date, include=include, max_pages=max_pages):
            ...
            try:
                ...
                if not home_team and len(participants)>0: home_team=participants[0].get("name")
                if not away_team and len(participants)>1: away_team=participants[1].get("name")
                kickoff_at=None; starting_at=fixture.get("starting_at")
                if starting_at:
                    try: kickoff_at=datetime.fromisoformat(str(starting_at).replace("Z", "+00:00"))
                    except ValueError: pass
                state=fixture.get("state") or {}; status=state.get("name") or state.get("state") or "scheduled"; league=fixture.get("league") or {}
                with SessionLocal.begin() as session:
                    session.execute(text("""INSERT INTO fixtures (fixture_id,provider,provider_fixture_id,league_name,home_team,away_team,kickoff_at,status,raw_json) VALUES (:fixture_id,'sportmonks',:provider_fixture_id,:league_name,:home_team,:away_team,:kickoff_at,:status,:raw_json) ON CONFLICT (provider,provider_fixture_id) DO UPDATE SET league_name=EXCLUDED.league_name,home_team=EXCLUDED.home_team,away_team=EXCLUDED.away_team,kickoff_at=EXCLUDED.kickoff_at,status=EXCLUDED.status,raw_json=EXCLUDED.raw_json,updated_at=NOW()"""), {"fixture_id":fixture_id,"provider_fixture_id":fixture_id,"league_name":league.get("name"),"home_team":home_team or "Unknown","away_team":away_team or "Unknown","kickoff_at":kickoff_at,"status":status,"raw_json":json.dumps(fixture,ensure_ascii=False,default=str)})
                await self.publisher.publish("provider.fixtures",fixture,f"sportmonks:fixture:{fixture_id}"); synced_fixture_ids.append(fixture_id); published += 1
