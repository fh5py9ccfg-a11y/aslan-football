...
class SportmonksClient:
    ...
    async def predictions_by_fixture(self, fixture_id, *, include="type"):
        ...
        payload = await self._request(
            "GET", f"/predictions/probabilities/fixtures/{fixture_id}", params=params
        )
        return tuple(payload.get("data") or ())

    async def events_by_fixture(self, fixture_id, *, page=1):
        payload = await self._request(
            "GET", f"/fixtures/{fixture_id}", params={"include": "events", "page": page}
        )
    ...
    async def fixtures_between(self, start_date, end_date, *, include=None, page=1):
        ...
        payload = await self._request(
            "GET", f"/fixtures/between/{start_date}/{end_date}", params=params
        )
        pagination = payload.get("pagination") or payload.get("meta", {}).get("pagination") or {}
        current = int(pagination.get("current_page", page))
        last = int(pagination.get("last_page", current))
